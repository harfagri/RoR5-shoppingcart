...
#  include PayPal::SDK::REST
  	 ...
  	 @my_payment = MyPayment.find_by(paypal_id: params[:paymentId])
  	 	if @my_payment.nil?
  	 		...
  	 		Stores::Paypal.checkout(params[:PayerID],params[:paymentId]) do
  	 			...
  	 			@my_payment.pay!
  	 			redirect_to ok_path,notice:"Pago procesado por PayPal"
  	 			return
  			end
  			redirect_to carrito_path,notice:"Error en proceso de pago por PayPal"
  		end
  ...
  def process_card
  	...
  	if paypal_helper.process_card(params).create
  	 ...
  	 @my_payment.pay!
  	 redirect_to ok_path, notice: "Pago realizado exitosamente"
  	else
